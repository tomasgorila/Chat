...
TriggerEvent('es:addCommand', 'stav', function(source, args, rawCommand)
  if source == 0 then
    print('esx_rpchat: you can\'t use this command from rcon!')
    return
  end
...
TriggerEvent('es:addCommand', 'lsc', function(source, args, rawCommand)
    local playerName = GetPlayerName(source)
--    local msg = rawCommand:sub(4)
    local xPlayer = ESX.GetPlayerFromId(source)
    local toSay = ''
    ...
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(135, 135, 135, 0.9); border-radius: 10px;"><i class="fas fa-wrench"></i> LSC: {0}</div>',
          args = {toSay}
      })
  else 
    TriggerClientEvent('chat:addMessage', source, {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(205, 0, 0, 0.9); border-radius: 10px;"><i class="fas fa-exclamation"></i>  Mus� pracovat u LSC pro mo�nost pou��t /lsc <i class="fas fa-exclamation"></i></div>',
      args = {}
    })
  end
end, false)